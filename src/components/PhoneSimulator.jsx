...
import { Play, Pause, RotateCcw, Heart, Zap, Volume2, Mountain, Users, Trophy, Activity, CheckCircle2, Gauge } from 'lucide-react';
...
export default function PhoneSimulator() {
  ...
  const [distance, setDistance] = useState(2.68);
  const [pace, setPace] = useState(278); // 4:38
  const [hr, setHr] = useState(162);
  const [ghostGap, setGhostGap] = useState(48);
  const [audioCue, setAudioCue] = useState('Tekan Mulai untuk menyalakan audio coach.');
  ...
  const toggleRun = () => {
    if (!isRunning) {
      audioEngine.playBeep(880, 'sine', 0.08);
      audioEngine.speak("Latihan dimulai. Target 5K pace sub-25 menit. Semangat!", "id-ID");
    } else {
      audioEngine.playBeep(440, 'sine', 0.08);
    }
  ...
  return (
    <section id="simulator" className="py-24 px-6 bg-[#FFF9F2] relative overflow-hidden">
      
      {/* Background Decorative Pattern */}
      <div className="absolute top-10 right-10 w-48 h-48 pattern-dots-orange opacity-40 pointer-events-none" />
      <div className="absolute bottom-10 left-10 w-48 h-48 pattern-dots-orange opacity-40 pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto space-y-16 relative z-10">
        ...
        <div className="text-center space-y-4 max-w-3xl mx-auto">
          ...
          <h2 className="text-3xl sm:text-5xl font-black text-gray-900">Simulasi Antarmuka iPhone 15 Pro</h2>
          <p className="text-gray-600 text-sm sm:text-base">
            Ganti tab untuk melihat modul aplikasi iOS StrideSync yang berjalan secara reaktif.
          </p>
        </div>

        {/* Tab Buttons */}
        <div className="flex flex-wrap items-center justify-center gap-2 max-w-xl mx-auto p-1.5 rounded-2xl bg-white shadow-md border border-orange-100">
          <button
            onClick={() => setActiveTab('hud')}
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-black transition flex items-center justify-center gap-2 ${activeTab === 'hud' ? 'bg-[#FF6A00] text-white shadow-md' : 'text-gray-600 hover:text-gray-900'}`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-black transition flex items-center justify-center gap-2 ${activeTab === 'feed' ? 'bg-[#FF6A00] text-white shadow-md' : 'text-gray-600 hover:text-gray-900'}`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-black transition flex items-center justify-center gap-2 ${activeTab === 'climbs' ? 'bg-[#FF6A00] text-white shadow-md' : 'text-gray-600 hover:text-gray-900'}`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-black transition flex items-center justify-center gap-2 ${activeTab === 'recovery' ? 'bg-[#FF6A00] text-white shadow-md' : 'text-gray-600 hover:text-gray-900'}`}
          >
        ...
        {/* Phone Container & Control Panel */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center max-w-5xl mx-auto">
          
          {/* Left: iPhone Mockup */}
          <div className="lg:col-span-6 flex justify-center">
            <div className="relative w-[320px] sm:w-[360px] p-4 bg-gray-900 rounded-[54px] border-4 border-gray-800 shadow-2xl shadow-orange-950/20">
              ...
              <div className="absolute top-7 left-1/2 -translate-x-1/2 w-28 h-7 bg-black rounded-full z-20 flex items-center justify-between px-3">
                <span className={`w-2.5 h-2.5 rounded-full ${isRunning ? 'bg-[#FF6A00] animate-pulse' : 'bg-gray-500'}`} />
                <span className="text-[10px] font-mono font-bold text-white">{isRunning ? 'REC' : 'IDLE'}</span>
              </div>
                ...
                {activeTab === 'hud' && (
                  <div className="space-y-4">
                    ...
                    <div className="text-center py-2">
                      <div className="text-5xl font-black tracking-tight font-mono">{distance.toFixed(2)}</div>
                      <div className="text-xs uppercase font-bold text-[#FF6A00] tracking-widest mt-1">Kilometer (KM)</div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                      <div className="bg-[#181B24] p-3.5 rounded-2xl border border-white/5 text-center">
                        ...
                        <div className="text-2xl font-black text-white font-mono mt-0.5">{formatPace(pace)}</div>
                      ...
                      <div className="bg-[#181B24] p-3.5 rounded-2xl border border-white/5 text-center">
                        ...
                        <div className="text-2xl font-black text-[#FF6A00] font-mono mt-0.5 flex items-center justify-center gap-1">
                          <Heart className="w-4 h-4 fill-current text-[#FF6A00] animate-pulse" />
                          <span>{hr}</span>
                        </div>
                    ...
                    <div className="space-y-2">
                      <div className="bg-[#181B24] p-3 rounded-2xl border border-orange-500/30 flex items-center justify-between">
                      ...
                      <div className="bg-[#181B24] p-3 rounded-2xl border border-white/5 flex items-center justify-between">
                        ...
                        <span className="text-xs font-mono font-bold text-[#FF6A00] bg-orange-500/10 px-2 py-1 rounded-lg">182 SPM</span>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="bg-[#181B24] p-3.5 rounded-2xl border border-white/5 space-y-2.5">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full bg-[#FF6A00] flex items-center justify-center font-bold text-xs">IS</div>
                          <div>
                            <div className="text-xs font-bold">Irsal Shydiq</div>
                            <div className="text-[9px] text-gray-400">Pagi ini di Senayan • Alphafly 3</div>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-3 gap-1 text-center bg-black/30 p-2 rounded-xl text-[10px]">
                        ...
                        <div><span className="text-gray-400 block">Pace</span><b className="text-[#FF6A00]">4'32"/km</b></div>
                        <div><span className="text-gray-400 block">Waktu</span><b className="text-white">45:28</b></div>
                      </div>
                      <div className="flex items-center justify-between text-[11px] text-gray-400 pt-1">
                        <span className="flex items-center gap-1 text-[#FF6A00] font-bold">❤️ 24 Kudos</span>
                ...
                {activeTab === 'climbs' && (
                  <div className="space-y-3">
                    ...
                    <div className="bg-[#181B24] p-4 rounded-2xl border border-[#2ECC71]/30 space-y-2">
                      ...
                      <div className="text-2xl font-black text-[#2ECC71] font-mono">+18 m <span className="text-xs text-gray-400 font-normal">Elevasi</span></div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-center text-[10px]">
                      <div className="bg-[#181B24] p-2.5 rounded-xl border border-white/5">
                        ...
                        <div className="text-lg font-black text-white font-mono mt-0.5">84.2</div>
                      </div>
                      <div className="bg-[#181B24] p-2.5 rounded-xl border border-white/5">
                        <div className="text-gray-400 font-semibold">Saran Istirahat</div>
                        <div className="text-lg font-black text-[#FF6A00] font-mono mt-0.5">18 Jam</div>
                      </div>
                    </div>
                  </div>
                )}

                {/* Bottom Hardware Navigation */}
                <div className="pt-3 border-t border-white/10 flex items-center justify-between text-xs text-gray-400 px-2 font-bold">
                  <span className={activeTab === 'hud' ? 'text-[#FF6A00]' : ''}>HUD</span>
                  <span className={activeTab === 'feed' ? 'text-[#FF6A00]' : ''}>Feed</span>
                  <span className={activeTab === 'climbs' ? 'text-[#FF6A00]' : ''}>Climbs</span>
                  <span className={activeTab === 'recovery' ? 'text-[#FF6A00]' : ''}>TRIMP</span>
                </div>
          ...
          {/* Right: Simulator Controls in Clean White Card */}
          <div className="lg:col-span-6 space-y-6">
            <div className="clean-white-card p-6 sm:p-8 space-y-6 border border-orange-100">
              
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-black text-gray-900">Kontrol Simulasi Lari</h3>
                  <p className="text-xs text-gray-500 font-semibold mt-0.5">Uji coba respons engine saat latihan lari</p>
                </div>
                <span className={`text-xs font-mono font-black px-3 py-1 rounded-full ${isRunning ? 'bg-green-100 text-[#2ECC71] border border-green-200' : 'bg-gray-100 text-gray-600'}`}>
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={toggleRun}
                  className={`flex-1 py-3.5 px-6 rounded-xl font-black text-sm flex items-center justify-center gap-2 shadow-lg transition ${isRunning ? 'bg-yellow-400 hover:bg-yellow-500 text-gray-900' : 'bg-[#FF6A00] hover:bg-[#E65100] text-white shadow-orange-500/25 hover:scale-105'}`}
                >
                ...
                <button
                  ...
                  className="py-3.5 px-4 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold text-sm transition flex items-center gap-1.5"
                >
              ...
              <div className="p-4 rounded-2xl bg-orange-50 border border-orange-200 space-y-1.5">
                <div className="flex items-center gap-2 text-xs font-black text-[#FF6A00] uppercase tracking-wider">
                  <Volume2 className="w-4 h-4" />
                  <span>Audio Voice Coach (AI Audio Cue)</span>
                </div>
                <div className="text-sm text-gray-800 italic font-semibold">
                  {audioCue}
                </div>
              </div>
